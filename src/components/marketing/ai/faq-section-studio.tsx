...
export function FAQSectionStudio() {
  return (
    <section className='py-8 sm:py-16 lg:py-24 bg-gray-50'>
      <div className='mx-auto max-w-7xl px-4 sm:px-6 lg:px-8'>
        {/* FAQ Header */}
        <div className='mb-12 space-y-4 text-center sm:mb-16 lg:mb-24'>
          <h2 className='text-2xl font-semibold md:text-3xl lg:text-4xl'>Häufige Fragen</h2>
        ...
        <Accordion type='single' collapsible className='w-full' defaultValue='item-1'>
          {faqItems.map((item, index) => (
            <AccordionItem key={index} value={`item-${index + 1}`} className='border border-gray-200 rounded-lg mb-4 bg-white shadow-sm'>
              <AccordionTrigger className='text-lg px-6 py-4'>{item.question}</AccordionTrigger>
